	...
	if (CRM.Mode == Save)
{
    ...
    if (Defined(record("quot_c_clonecompany")))
    {
        var RedirectUrl = "Clone/CloneQuote.asp?CompId=" +
            Server.URLEncode(String(record("quot_c_clonecompany")));

        if (Defined(record("quot_c_clonecontact")))
        {
            RedirectUrl += "&PID=" +
                Server.URLEncode(String(record("quot_c_clonecontact")));
        }

        if (Defined(record("quot_c_sageaccount")))
        {
            RedirectUrl += "&SageID=" +
                Server.URLEncode(String(record("quot_c_sageaccount")));
        }

        Response.Redirect(CRM.URL(RedirectUrl));
    }
}
else
{
